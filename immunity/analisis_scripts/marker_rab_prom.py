...
def plot_histograma_maduracion(all_sim_data, excluidas_dir, rab_columns, output_dir, porcentaje=50):
    """
    Histograma del tick en que cada simulación alcanza el porcentaje dado de RabD/area.
    Bins: <10k, 10-20k, ..., 50-60k, no maduraron. Subplots en columna.
    """
    ...
    bin_labels = ["<10k", "10-20k", "20-30k", "30-40k", "40-50k", "50-60k", "no maduraron"]

    # 3. Graficar subplots en columna
    n_grupos = len(ticks_mad)
    fig, axs = plt.subplots(n_grupos, 1, figsize=(10, 4*n_grupos), sharex=True)
    if n_grupos == 1:
        axs = [axs]
    for ax, (grupo, vals) in zip(axs, ticks_mad.items()):
        ...
        # Histograma normal
        counts, _ = np.histogram(madurados, bins=bin_edges)
        ...
        for i, c in enumerate(counts):
            ax.text(i, c + 0.1, str(c), ha='center', va='bottom', fontsize=10)
    axs[-1].set_xlabel(f"Tick de maduración (RabD/area ≥ {porcentaje}%)")
    plt.tight_layout()
