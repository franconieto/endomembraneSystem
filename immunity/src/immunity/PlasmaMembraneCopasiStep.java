...
public class PlasmaMembraneCopasiStep {
	// This class update the values of metabolites in the PM according to the time series generated by COPASI
...
//			
//						//System.out.print*ln("re calculate because uptake" );
//						try {
...
//					}
		...
		if (tick >= Collections.max(plasmaMembrane.getPlasmaMembraneTimeSeries().keySet())) {
			...
			callReceptorDynamics(plasmaMembrane);
//			//System.out.print*ln("re calculate se acabaron" );
//			try {
//			TimeUnit.SECONDS.sleep(5);
//		} catch (InterruptedException e) {
//			e.printStackTrace();
...
//		by area the membrane metabolites and by volume the soluble metabolites
		int tick = (int) RunEnvironment.getInstance().getCurrentSchedule().getTickCount();
//		//System.out.print*ln("tick "+ plasmaMembrane.getPlasmaMembraneTimeSeries().get(tick).get("R-TfEn"));
...
//		//System.out.print*ln("compartimiento volumen \t" + receptorDynamics.getModel().getCompartment(i).getObjectName() + receptorDynamics.getModel().getCompartment(i).getInitialValue());
        }

		Set<String> metabolites = receptorDynamics.getMetabolites();
		HashMap<String, Double> localM = new HashMap<String, Double>();
//		//System.out.print*ln("PM MEMBRENE RECYCLE " + plasmaMembrane.getMembraneRecycle());
		
		for (String met : metabolites) {
//			//System.out.print*ln("metabolito que no anda" + met);
...
////				//System.out.print*ln(Cell.area + "volume cell "+Cell.volume);
//				receptorDynamics.setInitialConcentration(met, sigFigs(metValue,6));
...
//			//System.out.print*ln(plasmaMembrane.getMembraneRecycle().get("pepMHCIEn")+" METABOLITES IN PM"+ localM);

	
////System.out.print*ln("LOCAL MMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMM " + localM);

		receptorDynamics.runTimeCourse();
		

		CTimeSeries timeSeries = receptorDynamics.getTrajectoryTask().getTimeSeries();
//		//System.out.print*ln("time series " + timeSeries);
		int stepNro = (int) timeSeries.getRecordedSteps();
		int metNro = metabolites.size();
		int tick = (int) RunEnvironment.getInstance().getCurrentSchedule().getTickCount();
//		//System.out.print*ln("steps "+stepNro+"metNro " + metNro  + " tick " + tick);
...
//			//System.out.print*ln("tick "+tick+"time " + time  + " time series " + value);
...
//			//System.out.print*ln("DELTA PM "+ delta);
