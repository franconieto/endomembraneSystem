...
//				proportional to the volume of endosome.  In brief, no need to consider a metLeft
		...
		Parameters parm = RunEnvironment.getInstance().getParameters(); //override NH3Cy concentration if it exists as a parameter
		if (parm != null && parm.getSchema().contains("NH3Cy")) {       //if not, it uses the concentration from the input
		    Object value = parm.getValue("NH3Cy");
		    double nh3_concentration=0;
		    //System.out.println(value);
		    if (value instanceof Number) {
		    	nh3_concentration = ((Number) value).doubleValue();
		    	lipidMetabolism.setInitialConcentration("NH3Cy", sigFigs(nh3_concentration,6));
		    	//System.out.println("NH3cy override " + nh3_concentration);		
		    }
		    
		}
		
		CModel model = lipidMetabolism.getModel();
		CModelValue acidPH = model.getModelValue("acidpH");
		CModelValue alkPH  = model.getModelValue("alkpH");
		
		//System.out.println("obtenido=acidph " + acidPH.getValue()+" alkph " + alkPH.getValue());
		
		if (parm != null && parm.getSchema().contains("acidpH") && parm.getSchema().contains("alkpH")) {       //if not, it uses the values in copasi
		    Object acid = parm.getValue("acidpH");
		    Object alk = parm.getValue("alkpH");
		    double acid_value=0;
		    double alk_value=0;
		    if (acid instanceof Number && alk instanceof Number) {
		    	acid_value = ((Number) acid).doubleValue();
		    	alk_value = ((Number) alk).doubleValue();
		    	acidPH.setValue(acid_value);
		    	alkPH.setValue(alk_value);
		    	
		    	acidPH.setInitialValue(acid_value);
		    	alkPH.setInitialValue(alk_value);

		    	//System.out.println("sobreescrito=acidph " + acid_value+" alkph " + alk_value);		
		    }
		...
		lipidMetabolism.runTimeCourse(endosome);
